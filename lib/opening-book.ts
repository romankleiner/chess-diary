 ...
 *
 * Looks up positions in a local Polyglot .bin file (no network calls).
 * Place a Polyglot book at  data/opening-book.bin  before running analysis.
 * Good free books: gm2600.bin, Titans.bin, gm2001.bin (see README or docs).
 *
 * Polyglot format reference: http://hgm.nubati.net/book_format.html
 * Zobrist table sourced from https://github.com/ddugovic/polyglot/random.c
 * via https://github.com/shaack/cm-polyglot  (MIT licence)
 */
...
// This exact table is the interoperability contract of the Polyglot format.
// Every compliant reader/writer uses these values unchanged.
//
// Layout:
//   [0  .. 767] RandomPiece[piece 0-11][square 0-63]  piece×64+square
//               piece index: 0=bP 1=wP 2=bN 3=wN 4=bB 5=wB
//                            6=bR 7=wR 8=bQ 9=wQ 10=bK 11=wK
//   [768..771]  RandomCastle  K Q k q
//   [772..779]  RandomEnPassant  file a-h
//   [780]       RandomTurn  (XOR'd when white to move)
const RANDOM64: bigint[] = [
  ...
  0xCF3145DE0ADD4289n, 0xF4F5D05C10CAB243n, 0x38B6525C21A42B0En, 0x36F60E2BA4FA6800n,
...
// Matches the KeyGenerator.js switch/case ordering
...
function computePolyglotKey(fen: string): bigint | null {
  const parts = fen.trim().split(' ');
  if (parts.length < 4) return null;
  const [board, turn, castling, epField] = parts;

  // Parse en-passant BEFORE scanning pieces — we need the column/row to
  // detect whether a pawn can actually capture (Polyglot only Xors ep when
  // a capturing pawn exists, unlike naive FEN interpretation).
  let epCol = -1, epRow = -1, canTakeEp = false;
  if (epField && epField !== '-' && epField.length === 2) {
    epCol = epField.charCodeAt(0) - 97; // 'a'=0 … 'h'=7
    epRow = epField.charCodeAt(1) - 49; // '1'=0 … '8'=7
  }
  ...
  for (let i = 0; i < 8; i++) {
    const rank = 7 - i; // FEN row 0 = rank 8 → Polyglot rank 7
    let col = 0;
    for (const ch of ranks[i]) {
      if (ch >= '1' && ch <= '8') {
        col += +ch;
      } else {
        const pi = PIECE_IDX[ch];
        if (pi === undefined) return null;
        hash ^= RANDOM64[pi * 64 + rank * 8 + col];
        // Check en-passant capturability while we visit pawns
        if (epCol >= 0) {
          if (ch === 'p' && epRow === 2 && rank === 3 &&
              (col === epCol - 1 || col === epCol + 1)) canTakeEp = true;
          if (ch === 'P' && epRow === 5 && rank === 4 &&
              (col === epCol - 1 || col === epCol + 1)) canTakeEp = true;
        }
        col++;
      }
  ...
  if (canTakeEp)           hash ^= RANDOM64[772 + epCol];
...
// Polyglot promotion: 0=none 1=n 2=b 3=r 4=q
const PROMOS = ['', 'n', 'b', 'r', 'q'];

// Polyglot encodes castling as king→rook-square (not king→landing-square):
//   e1h1 → e1g1  (white kingside)   e1a1 → e1c1  (white queenside)
//   e8h8 → e8g8  (black kingside)   e8a8 → e8c8  (black queenside)
const CASTLE_REMAP: Record<string, string> = {
  e1h1: 'e1g1', e1a1: 'e1c1',
  e8h8: 'e8g8', e8a8: 'e8c8',
};

function decodeMove(word: number): string {
  const toSq   =  word        & 0x3f;
  const fromSq = (word >> 6)  & 0x3f;
  const promo  = (word >> 12) & 0x7;
  const raw =
    FILES[fromSq % 8] + RANKS[Math.floor(fromSq / 8)] +
    FILES[toSq   % 8] + RANKS[Math.floor(toSq   / 8)] +
    PROMOS[promo];
  return CASTLE_REMAP[raw] ?? raw;
}

// ── Book file loading ─────────────────────────────────────────────────────
// Primary location: public/books/ — tracked by git, deployed to Vercel,
//   and included in the serverless function bundle via outputFileTracingIncludes.
// Fallback: data/ — for local dev if the file was placed there manually.
const BOOK_PATHS = [
  path.join(process.cwd(), 'public', 'books', 'opening-book.bin'),
  path.join(process.cwd(), 'data', 'opening-book.bin'),
];
const ENTRY_SIZE = 16; // key(8) + move(2) + weight(2) + learn(4)

let bookBuffer: Buffer | null = null;
let bookUnavailable = false; // avoid repeated fs.existsSync on missing file

function loadBook(): Buffer | null {
  if (bookBuffer) return bookBuffer;
  if (bookUnavailable) return null;

  const bookPath = BOOK_PATHS.find(p => fs.existsSync(p));
  if (!bookPath) {
    console.warn('[BOOK] No opening book found — book detection disabled.');
    console.warn('[BOOK] Expected at: public/books/opening-book.bin');
    bookUnavailable = true;
    return null;
  }

  const buf = fs.readFileSync(bookPath);
  if (buf.length % ENTRY_SIZE !== 0) {
    console.error(`[BOOK] ${bookPath}: size ${buf.length} is not a multiple of ${ENTRY_SIZE} — not a valid Polyglot file.`);
    bookUnavailable = true;
    return null;
  }

  bookBuffer = buf;
  console.log(`[BOOK] Loaded Polyglot book from ${bookPath}: ${buf.length / ENTRY_SIZE} entries`);
  return bookBuffer;
}

// ── Binary search ─────────────────────────────────────────────────────────
function findBookMoves(key: bigint, buf: Buffer): string[] {
  const total = Math.floor(buf.length / ENTRY_SIZE);
  let lo = 0, hi = total - 1;

  while (lo <= hi) {
    const mid = Math.floor((lo + hi) / 2);
    const entryKey = buf.readBigUInt64BE(mid * ENTRY_SIZE);
    if (entryKey === key) {
      // Scan back to find the first entry with this key
      let start = mid;
      while (start > 0 && buf.readBigUInt64BE((start - 1) * ENTRY_SIZE) === key) start--;
      // Collect all moves for this position (weight > 0 only)
      const moves: string[] = [];
      for (let i = start; i < total; i++) {
        if (buf.readBigUInt64BE(i * ENTRY_SIZE) !== key) break;
        const moveWord = buf.readUInt16BE(i * ENTRY_SIZE + 8);
        const weight   = buf.readUInt16BE(i * ENTRY_SIZE + 10);
        if (weight > 0) moves.push(decodeMove(moveWord));
      }
      return moves;
...
/**
 * Returns true if `uciMove` (e.g. "d7d6") appears in the local Polyglot
 * opening book for the given position.  The book file is loaded from disk
 * once and cached in memory; subsequent calls are pure in-memory lookups.
 *
 * If the book file is absent all moves return false (analysis continues
 * without book detection rather than crashing).
 */
export async function isBookMove(fen: string, uciMove: string): Promise<boolean> {
  const buf = loadBook();
  if (!buf) return false;
  const key = computePolyglotKey(fen);
  if (key === null) {
    console.warn(`[BOOK] Could not compute Zobrist key for FEN: ${fen}`);
    return false;
  }
  return findBookMoves(key, buf).includes(uciMove);
}

/** Drop the cached book buffer (e.g. after swapping the .bin file). */
...
/** @deprecated */
export async function isInBook(_fen: string, _minGames = 50): Promise<boolean> { return false; }
/** @deprecated */
export async function countBookMoves(_pgn: string, _minGames = 50): Promise<number> { return 0; }
